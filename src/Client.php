...
use PagarMe\HttpClient;
use PagarMe\Configuration;
use PagarMe\Exception;
...
class Client
{
    ...
    public function __construct($apiKey, array $extras = null)
    {
        ...
        $this->http->config->setHost(self::BASE_URI);
        $this->http->config->setUserAgent('PagarMe_SDK/1.0.0/php');
        $this->http->config->addHeader('Content-Type','application/json');

        if (!is_null($extras['headers'])) {
            $this->http->config->addHeaders($extras['headers']);
        }
    ...
    {		
		try {
            list($response, $statusCode, $httpHeader) = $this->http->call(
                $uri,
                $method,
